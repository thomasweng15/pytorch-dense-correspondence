...
def batch_find_pixel_correspondences(img_a_knots, img_b_knots, img_a_mask=None, uv_a=None, num_attempts=10, device='CPU'):
    ...
    if uv_a is None:
	"randomly picking"
        # No pixels provided, randomly sample from mesh vertex pixels
        uv_a = pytorch_rand_select_knot_pixels(img_a_knots, num_samples=num_attempts)
    else:
    ...
    uv_a_coords = [list(c) for c in zip(uv_a_vec_list[0], uv_a_vec_list[1])]
    uv_a_res = torch.Tensor(uv_a_vec_list).type(dtype_long)
    uv_a_res = (uv_a_res[0], uv_a_res[1])
    if img_a_mask is not None:
        # if we are sampling from mask, take each sampled pixel, convert to closest knot pixel
        uv_a_knots = pixels_to_closest_knot(uv_a_coords, img_a_knots)
    else:
        uv_a_knots = uv_a_coords
    uv_b_list_x, uv_b_list_y = [], []
...
def btch_find_pixel_correspondences_old(img_a_depth, img_a_pose, img_b_depth, img_b_pose,
