...
class HeatmapVisualization(object):
    ...
    def _compute_descriptors(self, knot_idx):
        ...
        source, blended, target = self.find_best_match(None, u, v, None, None)
        return (source, blended, target)
    ...
    def run(self):
	    ...
	    print "computing descriptors"
            source, blended, target = self._compute_descriptors(i)
	    print "computed descriptors"
            vis = np.concatenate((source, target), axis=1)
	    print "concatenated, writing image"
	    cv2.imwrite("/home/priya/code/data_volume/annotated/%06d.png" % i, vis)
	    #cv2.imwrite("/home/priya/code/data_volume/annotated/%06d_heatmap.png" % i, blended)
