...
#	self._image_dir = '/home/priya/code/data_volume/pdc_rope/logs_proto/rope_blender_images'
    ...
    def load_specific_dataset(self):
        dataset_config_filename = os.path.join(utils.getDenseCorrespondenceSourceDir(), 'config',
                                               ...
                                               'dataset', 'composite', 'rope_nonrigid_only.yaml')

        dataset_config = utils.getDictFromYamlFilename(dataset_config_filename)
        self._dataset = SpartanDataset(config=dataset_config)

    def get_random_image_pair(self, dir=None):
...
#        if self._config["randomize_images"]:
    ...
    def _get_new_images(self):
        ...
        if not self._image_dir:
	        ...
	        print "dataset in train mode"
                self._dataset.set_train_mode()
            else:
	        print "dataset in test mode"
                self._dataset.set_test_mode()

            if self._config["same_object"]:
	        print "same object config mode"
                scene_name_1, scene_name_2, image_1_idx, image_2_idx = self.get_random_image_pair()
	        print "got random image pair"
	        print (scene_name_1, scene_name_2, image_1_idx, image_2_idx)
	    ...
	    self.img1_pil = Image.open(os.path.join(self._image_dir, random.choice(os.listdir(self._image_dir)))).convert('RGB') 
            self.img2_pil = Image.open(os.path.join(self._image_dir, random.choice(os.listdir(self._image_dir)))).convert('RGB') 
